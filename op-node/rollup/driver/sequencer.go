...
func NewSequencer(log log.Logger, cfg *rollup.Config, l1 Downloader, l2 derive.Engine, engineState EngineState, l1OriginSelector L1OriginSelectorIface, metrics SequencerMetrics) *Sequencer {
	return &Sequencer{
		log:              log,
		config:           cfg,
		l1:               l1,
...
func (d *Sequencer) StartBuildingBlock(ctx context.Context, l1Head eth.L1BlockRef) error {
	...
	if l1Origin.Number < d.config.Genesis.L1.Number {
		return fmt.Errorf("the L1 origin %s cannot be before genesis at %s", l1Origin, d.config.Genesis.L1)
	}
